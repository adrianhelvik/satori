...
| `list-style-type` | **Supported (approx.)** | Supports common marker styles (`disc`, `circle`, `square`, `decimal`, `lower-hexadecimal`, `upper-hexadecimal`, alpha, roman), additive scripts (`armenian`, `lower-armenian`, `upper-armenian`, `georgian`), alphabetic scripts (`lower-cyrillic`, `upper-cyrillic`, `lower-greek`), disclosure markers (`disclosure-open` / `disclosure-closed`), and quoted string markers (`"→"`). |
