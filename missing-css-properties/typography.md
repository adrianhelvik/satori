...
- `word-spacing`, `text-indent` (length/percentage values plus `each-line` and `hanging` keywords, approximate)
...
| Property | Feasibility | Notes |
|----------|-------------|-------|
| `text-indent` | **Supported (approx.)** | Supports length/percentage values and keyword flags `each-line` and `hanging`. Writing-mode and bidi-dependent behavior remains limited. |
